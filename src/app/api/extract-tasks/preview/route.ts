      ...
      content: [
        {
          type: 'text',
          text: [
            'TODAY IS: ' + new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) + ' (' + new Date().toISOString().split('T')[0] + ')',
            'DATES: All deadlines must be FUTURE. "Thursday" = coming Thursday, "next week" = 7+ days, "tomorrow" = tomorrow. Never use past dates.',
            projectCheck.current >= projectCheck.limit ? 'NOTE: Project limit reached (' + projectCheck.limit + '). Use "General" project only.' : '',
            '',
            'Extract all tasks from this text. Group into logical projects.',
            '',
            'Text: "' + rawText + '"',
            '',
