...
import { Plus, Filter, Sparkles, Download } from 'lucide-react'
...
export default function TasksPage() {
  ...
  function exportCSV() {
    const url = filter !== 'all' ? `/api/export-tasks?status=${filter}` : '/api/export-tasks'
  ...
  const grouped = tasks.reduce<Record<string, Task[]>>((acc, task) => {
    const key = (task as Task & { project?: { name: string } }).project?.name ?? 'No project'
    if (!acc[key]) acc[key] = []
    acc[key].push(task)
    return acc
  }, {})
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className="flex items-center gap-1.5 text-sm text-[#2d7a4f] border border-[#2d7a4f]/30 px-3 py-1.5 rounded-lg hover:bg-[#e8f5ee] transition-colors"
          >
            <Sparkles size={14} />
            AI add
          </Link>
...
function TaskRow({
  ...
  task: Task & { project?: { name: string; colour: string } }
  onToggle: (t: Task) => void
}) {
  return (
    <div className="flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-gray-100 hover:border-[#2d7a4f]/20 task-card group">
      <input
        type="checkbox"
