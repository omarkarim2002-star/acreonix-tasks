...
import { Play, Square, Loader2, CheckCircle2, Sparkles, Timer } from 'lucide-react'
...
export default function TrackerPage() {
  ...
  const [tab, setTab] = useState<'timer' | 'natural'>('timer')
  const [apiError, setApiError] = useState('')

  useEffect(() => {
    Promise.all([
      fetch('/api/time-logs?days=7').then(r => r.json()).catch(() => []),
      fetch('/api/tasks').then(r => r.json()).catch(() => []),
    ]).then(([l, t]) => {
      setLogs(Array.isArray(l) ? l : [])
      setTasks(Array.isArray(t) ? t.filter((t: any) => t.status !== 'done') : [])
      if (!Array.isArray(l) && l?.error) {
        setApiError('Time logs table not set up yet — please run supabase-phase4.sql in your Supabase SQL editor.')
      }
      setLoading(false)
    })
  }, [])
  ...
  return (
    <div className="px-6 py-8 max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm mt-3">Log real time spent. The more you log, the smarter AI scheduling gets.</p>
      </div>
      ...
      {activeTimer && (
        <div className="text-white rounded-2xl px-5 py-4 mb-6 flex items-center justify-between" style={{ background: '#2d7a4f' }}>
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={stopTimer} disabled={submitting} className="flex items-center gap-2 bg-white text-sm font-semibold px-4 py-2 rounded-xl hover:bg-white/90" style={{ color: '#2d7a4f' }}>
              <Square size={14} />Stop
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6 shadow-sm">
        <div className="flex border-b border-gray-100">
          {[{ key: 'timer', label: 'Live timer', icon: Timer }, { key: 'natural', label: 'Natural language', icon: Sparkles }].map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => setTab(key as any)} className={cn('flex-1 flex items-center justify-center gap-2 py-3 text-xs font-medium transition-colors', tab === key ? 'text-[#2d7a4f] bg-[#e8f5ee]' : 'text-gray-500 hover:bg-gray-50')}>
              <Icon size={13} />{label}
            </button>
          ))}
        </div>
        <div className="p-5">
          <div className="mb-4">
            ...
            <select value={selectedTask} onChange={e => setSelectedTask(e.target.value)} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-[#2d7a4f] bg-white">
              <option value="">Select a task (optional)</option>
              {tasks.map(t => <option key={t.id} value={t.id}>{(t as any).project?.name ? `[${(t as any).project.name}] ` : ''}{t.title}</option>)}
            </select>
          </div>
          {tab === 'timer' && (
            <button onClick={activeTimer ? stopTimer : startTimer} disabled={submitting || (!activeTimer && !selectedTask)}
              className={cn('w-full flex items-center justify-center gap-2 text-sm font-semibold py-3 rounded-xl transition-all disabled:opacity-50', activeTimer ? 'bg-red-500 text-white hover:bg-red-600' : 'text-white')}
          ...
          {tab === 'natural' && (
            <div>
              ...
              <textarea value={naturalInput} onChange={e => setNaturalInput(e.target.value)}
                placeholder='"worked on the proposal from 9am to 10:30" or "spent about 45 mins on client emails just now"'
                rows={3} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-[#2d7a4f] resize-none mb-3" />
              <button onClick={submitNatural} disabled={!naturalInput.trim() || submitting}
                className="w-full flex items-center justify-center gap-2 text-white text-sm font-semibold py-2.5 rounded-xl disabled:opacity-50" style={{ background: '#2d7a4f' }}>
                ...
                <div key={log.id} className="bg-white rounded-xl border border-gray-100 px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">
                      {isToday ? 'Today' : start.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} · {start.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                  ...
                  <div className="text-right shrink-0">
                    {log.duration_minutes ? (
                      <span className="text-sm font-semibold text-gray-700">
                        {log.duration_minutes >= 60 ? `${Math.floor(log.duration_minutes / 60)}h ${log.duration_minutes % 60}m` : `${log.duration_minutes}m`}
                      </span>
