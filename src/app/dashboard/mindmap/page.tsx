...
function RootNode({ data }: NodeProps) {
  return (
    <div style={{ background: '#fff', border: '2px solid #2d7a4f', borderRadius: 16, padding: '12px 24px', textAlign: 'center', minWidth: 160, boxShadow: '0 4px 16px rgba(45,122,79,0.12)' }}>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: '#2d7a4f', letterSpacing: 3, fontFamily: 'Georgia,serif' }}>ACREONIX</div>
...
function ProjectNode({ data }: NodeProps) {
  ...
  return (
    <div onClick={() => router.push(`/dashboard/projects/${data.projectId}`)}
      ...
      <div style={{ fontSize: 12, fontWeight: 700, color: '#141b2d', fontFamily: 'Georgia,serif' }}>{data.label}</div>
...
export default function GlobalMindMapPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (Array.isArray(projs) && Array.isArray(tasks)) {
        setProjects(projs.map((p: any) => ({ ...p, tasks: tasks.filter((t: any) => t.project_id === p.id) })))
      }
      setLoading(false)
    })
  }, [])
  ...
  const { nodes: initNodes, edges: initEdges } = useMemo(() => {
    ...
    const spacing = Math.max(280, Math.min(340, 1200 / projects.length))
  ...
  if (loading) return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, background: '#f4f6f8' }}>
    ...
    <div style={{ position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: '#f4f6f8', zIndex: 40 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 24px', background: '#fff', borderBottom: '1px solid #e8edf2', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#141b2d', fontFamily: 'Georgia,serif' }}>All projects — mind map</span>
          <span style={{ fontSize: 12, color: '#9aa3b4' }}>{projects.length} projects · {projects.reduce((a, p) => a + p.tasks.length, 0)} tasks</span>
        </div>
      ...
      {/* React Flow — position:fixed + explicit inset gives it a real pixel box */}
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        {projects.length === 0 ? (
          ...
          >
            ...
            <MiniMap nodeColor={(n) => n.type === 'projectNode' ? (n.data as any).colour : STATUS_DOT[(n.data as any).status] ?? '#d1d5db'} maskColor="rgba(244,246,248,0.8)" />
          </ReactFlow>
        )}
      </div>
    </div>
