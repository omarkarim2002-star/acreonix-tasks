      ...
      <div className="grid grid-cols-3 gap-4 mb-8 fade-up fade-up-2">
        {[
          ...
          <div key={label} className="rounded-xl px-5 py-4" style={{ background: bg }}>
            ...
            <p className="text-2xl font-semibold" style={{ fontFamily: 'DM Serif Display, Georgia, serif', color: '#141b2d' }}>{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 fade-up fade-up-3">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/dashboard/tasks" className="flex items-center gap-1 text-xs font-medium transition-colors" style={{ color: '#2d7a4f' }}>
          ...
          <div className="space-y-1.5">
            {tasks.slice(0, 6).map((task, i) => (
              ...
              >
                ...
                {task.deadline && (
                  <span className={`text-xs shrink-0 ${deadlineColour(task.deadline)}`}>{formatDeadline(task.deadline)}</span>
                )}
              </Link>
            ))}
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/dashboard/projects" className="flex items-center gap-1 text-xs font-medium transition-colors" style={{ color: '#2d7a4f' }}>
              View all <ArrowRight size={11} />
            </Link>
          </div>
          <div className="space-y-1.5">
                ...
                <div key={p.id} className="task-row overflow-hidden">
                  <div className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <div className="h-1 flex-1 rounded-full overflow-hidden" style={{ background: '#e8edf2' }}>
                            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: p.colour }} />
                          </div>
                    ...
                    <Link
                      ...
                      className="flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded-md transition-colors shrink-0"
                      style={{ color: '#9aa3b4' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = '#2d7a4f'; (e.currentTarget as HTMLElement).style.background = '#e8f4ee' }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = '#9aa3b4'; (e.currentTarget as HTMLElement).style.background = 'transparent' }}
                    >
                      <GitFork size={10} />map
                    </Link>
                  </div>
