      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <section className="col-span-3">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/tasks" className="text-sm font-semibold" style={{ color:'#0D3D2E' }}>All tasks →</Link>
          </div>

          <div className="rounded-2xl overflow-hidden" style={{ background:'#fff', boxShadow:'0 4px 16px rgba(16,19,18,0.08)' }}>
            {focus.length === 0 ? (
              <div className="py-12 text-center text-sm" style={{ color:'#9BA5A0' }}>All caught up — nothing urgent</div>
            ) : focus.map((task, i) => {
              ...
              return (
                <Link key={task.id} href={`/dashboard/tasks/${task.id}`}
                  ...
                  style={{ borderLeft:`3px solid ${colour}`, borderBottom: i < focus.length-1 ? '1px solid #F7F8F5' : 'none' }}
                >
                  <div className="w-5 h-5 rounded-full border-2 shrink-0" style={{ borderColor:colour }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: isOverdue ? '#DC2626' : '#101312' }}>{task.title}</p>
                    {(proj || task.deadline) && (
                      <div className="flex items-center gap-2 mt-0.5">
                        {proj && <span className="text-xs" style={{ color:'#9BA5A0' }}>{proj.name}</span>}
                        {task.deadline && <span className="text-xs font-medium" style={{ color: isOverdue ? '#DC2626' : '#9BA5A0' }}>{isOverdue ? '⚠ ' : ''}{formatDeadline(task.deadline)}</span>}
                      </div>
                    )}
                  </div>
        ...
        <section className="col-span-2 flex flex-col gap-4">
          ...
          <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background:'#0D3D2E', boxShadow:'0 4px 14px rgba(13,61,46,0.22)' }}>
            ...
            <Link href="/dashboard/extract" className="flex items-center gap-1.5 text-sm font-black px-4 py-2 rounded-full shrink-0" style={{ background:'#D7F36A', color:'#071F17' }}>
              ✦ Plan
            </Link>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/dashboard/projects" className="text-sm font-semibold" style={{ color:'#0D3D2E' }}>All →</Link>
            </div>
            <div className="space-y-2">
              ...
              {projArr.length === 0 && <p className="text-sm text-center py-6" style={{ color:'#9BA5A0' }}>No projects yet</p>}
            </div>
          </div>
        </section>
      </div>
