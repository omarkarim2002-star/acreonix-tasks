      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {projects.map(project=>{
            ...
            return (
              <Link key={project.id} href={`/dashboard/projects/${project.id}`} className="bg-white rounded-2xl border border-gray-100 px-5 py-4 hover:border-[#2d7a4f]/30 hover:shadow-sm
...
export default function ProjectDetailPage() {
  ...
  useEffect(()=>{
    Promise.all([fetch(`/api/projects/${id}`).then(r=>r.json()),fetch(`/api/tasks?project_id=${id}`).then(r=>r.json())]).then(([proj,t])=>{
      setProject(proj);setNameValue(proj.na
cat > "src/app/dashboard/projects/[id]/page.tsx" << 'EOF'
'use client'
import { useState, useEffect } from 'react'
import { useParams, useRouter } from 'next/navigation'
