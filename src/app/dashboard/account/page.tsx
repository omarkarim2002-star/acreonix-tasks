...
import { Clock, Coffee, CreditCard, LogOut, Save, Camera, ChevronRight, X } from 'lucide-react'
...
export default function AccountPage() {
  ...
  return (
    <div className="px-8 py-8 max-w-2xl mx-auto">
      ...
      {/* Profile card — click to open Clerk UserProfile modal */}
      <div className="rounded-2xl p-6 mb-6" style={{ background:'#fff', boxShadow:'0 2px 8px rgba(16,19,18,0.06)' }}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-xl font-black" style={{ color:'#101312' }}>{fullName || 'Your account'}</p>
            <p className="text-sm mt-0.5" style={{ color:'#9BA5A0' }}>{user?.primaryEmailAddress?.emailAddress}</p>
          </div>

          <button
            onClick={() => openUserProfile()}
            className="text-sm font-semibold px-4 py-2 rounded-xl transition-all"
            style={{ background:'#F7F8F5', color:'#66706B' }}
          >
            Edit profile
          </button>
        </div>
      ...
      <div className="rounded-2xl p-6 mb-6" style={{ background:'#fff', boxShadow:'0 2px 8px rgba(16,19,18,0.06)' }}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-5">
          <div>
            <label className="text-xs font-bold mb-2 block" style={{ color:'#9BA5A0', letterSpacing:'0.8px' }}>START TIME</label>
          ...
          <div>
            <label className="text-xs font-bold mb-2 block" style={{ color:'#9BA5A0', letterSpacing:'0.8px' }}>END TIME</label>
        ...
        <div className="mb-5">
          <label className="text-xs font-bold mb-2 block" style={{ color:'#9BA5A0', letterSpacing:'0.8px' }}>
            <Coffee size={11} className="inline mr-1" />LUNCH BREAK
          </label>
          <div className="flex gap-2">
            {BREAK_OPTS.map(opt => (
              <button key={opt.value} onClick={() => setBreakTime(opt.value)}
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background:'#fff', boxShadow:'0 2px 8px rgba(16,19,18,0.06)' }}>
        ...
        <button onClick={() => signOut(() => router.push('/'))}
          className="flex items-center gap-3 px-5 py-4 w-full hover:bg-red-50 transition-colors">
